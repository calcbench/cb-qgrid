...
class QgridWidget(widgets.DOMWidget):
    ...
    def _update_table(
        ...
        df_json = pd_json.to_json(
            None, df, orient="table", date_format="iso", double_precision=self.precision
        )
    ...
    def _get_col_series_from_df(self, col_name, df: pd.DataFrame, level_vals=False):
        sort_column_name = self._sort_helper_columns.get(col_name)
        if sort_column_name:
