...
from pandas.io.common import get_filepath_or_buffer, _get_handle, _stringify_path
...
class SeriesWriter(Writer):
    ...
    def _format_axes(self):
        if not self.obj.index.is_unique and self.orient == "index":
            raise ValueError(
                "Series index must be unique for orient=" "'%s'" % self.orient
            )
...
class FrameWriter(Writer):
    ...
    def _format_axes(self):
        ...
        if not self.obj.index.is_unique and self.orient in ("index", "columns"):
            raise ValueError(
                "DataFrame index must be unique for orient=" "'%s'." % self.orient
            )
        if not self.obj.columns.is_unique and self.orient in (
            ...
            raise ValueError(
                "DataFrame columns must be unique for orient=" "'%s'." % self.orient
            )
...
def read_json(
    ...
    if isinstance(filepath_or_buffer, compat.string_types):
        ...
        if exists:
            fh, handles = _get_handle(filepath_or_buffer, "r", encoding=encoding)
            json = fh.read()
            fh.close()
        else:
...
class Parser(object):
    ...
    def __init__(
        ...
        if date_unit is not None:
            ...
            if date_unit not in self._STAMP_UNITS:
                raise ValueError("date_unit must be one of %s" % (self._STAMP_UNITS,))
    ...
    def check_keys_split(self, decoded):
        ...
        if bad_keys:
            ...
            raise ValueError(
                u("JSON data had unexpected key(s): %s") % pprint_thing(bad_keys)
            )
